...
const fetch = require('node-fetch'); // for Coingate API calls
...
app.post('/api/register', (req, res) => {
  ...
  const user = {
    ...
    crypto: {
      BTC: Number(process.env.STARTER_BTC || 0.0005),
      ETH: Number(process.env.STARTER_ETH || 0.01)
    },
    createdAt: Date.now()
  };
...
app.post('/api/convert', (req, res) => {
  ...
  if (!userId || typeof coins === 'undefined')
    return res.status(400).json({ ok: false, error: 'userId and coins required' });
...
app.post('/api/coinbase-charge', (req, res) => {
  ...
  const coinsNum = Number(coins);
  if (coinsNum < 2000 || coinsNum > 10500)
    return res.status(400).json({ ok: false, error: 'coins must be 2000–10500' });
  if (user.coins < coinsNum)
    return res.status(400).json({ ok: false, error: 'insufficient coins' });
...
// ---------------- Test Coingate API Key ----------------
app.get('/test-coingate', async (req, res) => {
  if (!process.env.COINGATE_API_KEY) {
    return res.status(500).json({ ok: false, message: "COINGATE_API_KEY not set" });
  }

  try {
    const response = await fetch('https://api.coingate.com/v2/rates/merchant/BTC/USD', {
    ...
    res.json({ ok: true, message: "Coingate key works!", rate: data });
  } catch (err) {
    console.error("Coingate test failed:", err.message);
    res.status(500).json({ ok: false, message: "Failed to reach Coingate", error: err.message });
  }
});
